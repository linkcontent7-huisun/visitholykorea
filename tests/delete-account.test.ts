...
describe('계정 삭제', () => {
  it('중첩 폴더의 사진을 모두 지운 뒤 연결 기록과 계정을 삭제한다', async () => {
    ...
    const entries: Record<string, { name: string; id: string | null }[]> = {
      user: [{ name: 'site.jpg', id: 'photo' }, { name: 'stamp', id: null }],
    ...
    const deps = {
      ...
      removePhotos: vi.fn(async (paths: string[]) => { calls.push(`remove:${paths.join(',')}`); }),
      deleteRows: vi.fn(async (table: string) => { calls.push(`rows:${table}`); }),
      deleteUser: vi.fn(async () => { calls.push('user'); }),
    };

    await deleteAccountData('user', deps);

    expect(deps.removePhotos).toHaveBeenCalledWith(['user/site.jpg', 'user/stamp/1.jpg']);
    expect(calls.indexOf('user')).toBeGreaterThan(calls.indexOf('remove:user/site.jpg,user/stamp/1.jpg'));
  ...
  it('사진 삭제가 실패하면 계정을 삭제하지 않는다', async () => {
    const deps = {
      ...
      removePhotos: vi.fn(async () => { throw new Error('storage failed'); }),
  ...
  it('연결 기록 삭제가 실패하면 계정을 삭제하지 않는다', async () => {
    const deps = {
      ...
      deleteRows: vi.fn(async () => { throw new Error('database failed'); }),
      deleteUser: vi.fn(async () => {}),
    };
