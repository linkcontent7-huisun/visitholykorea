...
async function tour<T>(service: string, op: string, params: Record<string, string | number>): Promise<{ total: number; items: T[] }> {
  const q = new URLSearchParams({ serviceKey: key!, MobileOS: 'ETC', MobileApp: 'vhk', _type: 'json', ...Object.fromEntries(Object.entries(params).map(([k, v]) => [k, String(v)])) });
  calls += 1;
  const res = await fetch(`${BASE}/${service}/${op}?${q}`);
  const text = await res.text();
  let json: { response?: { header?: { resultMsg?: string }; body?: { totalCount?: number; items?: { item?: T[] } } } };
  try {
...
// 1. 법정동 코드표 — 시·도, 시군구
interface LdongCode { code: string; name: string }
const sido = (await tour<LdongCode>('KorService2', 'ldongCode2', { numOfRows: 50, pageNo: 1 })).items;
const signguBySido = new Map<string, LdongCode[]>();
for (const s of sido) {
  const list = (await tour<LdongCode>('KorService2', 'ldongCode2', { numOfRows: 100, pageNo: 1, lDongRegnCd: s.code })).items;
...
function shortSido(name: string): string[] {
  const m: Record<string, string[]> = {
    ...
    충청북도: ['충북', '충청북도'], 충청남도: ['충남', '충청남도'], 전북특별자치도: ['전북', '전라북도'],
    // 2026 행정구역 개편으로 광주·전남이 합쳐졌다. 성지 주소는 아직 옛 이름이다.
    전남광주통합특별시: ['전남', '전라남도', '광주'], 경상북도: ['경북', '경상북도'], 경상남도: ['경남', '경상남도'], 제주특별자치도: ['제주'],
  };
  return [name, ...(m[name] ?? [])];
}
...
const { data: sites, error } = await supabase.from('holy_sites').select('id, name, location').order('name');
if (error || !sites) {
...
interface Located { name: string; address: string | null; areaCd: string; signguCd: string; signguNm: string }
...
for (const site of sites) {
  ...
  const g = [...list].sort((a, b) => b.name.length - a.name.length).find((x) => addr.includes(x.name));
  if (s && g) located.push({ name: site.name, address: site.location, areaCd: s.code, signguCd: s.code + g.code, signguNm: `${s.name} ${g.name}` });
  else unlocated.push({ name: site.name, address: site.location });
}

// 3. 시군구별 집중률 관광지 이름 (전체 페이지)
interface Rate { tAtsNm: string; baseYmd: string; cnctrRate: string }
...
for (const { areaCd, signguCd } of uniqueSigngu) {
  ...
  for (let pageNo = 1; pageNo <= 40; pageNo += 1) {
    const { total, items } = await tour<Rate>('TatsCnctrRateService', 'tatsCnctrRatedList', { areaCd, signguCd, numOfRows: 300, pageNo });
...
for (const l of located) {
  ...
  const a = norm(l.name);
  const hit = names.find((n) => norm(n) === a) ?? names.find((n) => { const b = norm(n); return b.length >= 3 && (a.includes(b) || b.includes(a)); });
...
for (const m of matched) console.log(`  ${m.site}${m.site === m.spot ? '' : `  ← "${m.spot}"`}  (${m.signgu})`);
console.log(`\n▶ 같은 시군구에 관광지 데이터는 있지만 성지 이름은 없음: ${unmatched.filter((u) => (namesBySigngu.get(u.signguCd)?.size ?? 0) > 0).length}곳`);
console.log(`▶ 시군구 자체에 집중률 데이터 없음: ${unmatched.filter((u) => (namesBySigngu.get(u.signguCd)?.size ?? 0) === 0).length}곳`);
if (unlocated.length) {
