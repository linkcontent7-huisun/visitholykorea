...
if (!url || !key)
  throw new Error('.env.local 에 VITE_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY 가 필요하다');
...
for (const it of items) {
  ...
  if (upErr) {
    console.error('✗ 업로드 실패', it.siteId, upErr.message);
    continue;
  }
  const pub = sb.storage.from('site-photos').getPublicUrl(path).data.publicUrl + `?v=${Date.now()}`;
  const { error } = await sb
    .from('holy_sites')
    .update({ image_url: pub, image_source: it.source, image_license: it.license })
    .eq('id', it.siteId);
  if (error) {
    console.error('✗ DB 갱신 실패', it.siteId, error.message);
    continue;
  }
  await sb.from('site_sources').upsert(
    {
      site_id: it.siteId,
      kind: 'field',
      title: `대표사진: ${it.source}`,
      url: null,
      collected_at: new Date().toISOString().slice(0, 10),
      collected_by: 'Claude',
      note: `원본 ${(it as Item & { orig?: string }).orig ?? it.file}`,
    },
