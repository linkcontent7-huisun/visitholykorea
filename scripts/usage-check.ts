...
const QUERIES: Array<[string, string]> = [
  ...
  [
    '최근 30일 가입',
    `select count(*)::int as n from auth.users where ${REAL_USER} and created_at > now() - interval '30 days'`,
  ],
  ...
  [
    '한 줄 기록',
    `select count(*)::int as n from pilgrimage_stamps where note is not null and note <> ''`,
  ],
