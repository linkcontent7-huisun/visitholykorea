...
rows = [
    ...
    ['웹 서비스 (반응형 PWA · 홈 화면 설치 지원) — 같은 코드로 Android/iOS 앱(Capacitor) 빌드'],
    ...
    [f'성지 {SITES}곳의 68%가 비수도권에 있고 대부분 조용하다. 순례자는 목적지가 정해진 여행자라 "명동 성당 다음엔 어디"로 물으면 동선을 바꿀 수 있다. '
...
set_text(t.cell(0, 1), [
    '1) 관광공사 「관광지 집중률 예측」으로 성지 인근 시·군·구의 붐빔을 그날 기준 조용·보통·붐빔 3단계로 보여 준다 — 숫자 대신 문장으로, 50대 이상도 한눈에 읽히게.',
    '2) 사용자의 마음·출발지·시간(질문 3개)에 맞는 후보 성지를 고르고, 하나를 축으로 오전 성지 → 점심(주변 식당) → 오후(주변 관광지·걷기길)의 하루 일정을 TourAPI 실시간 조회로 짠다. 응답은 저장하지 않는다.',
    f'3) 현장 체류의 질을 높이는 콘텐츠 — 성지 {SITES}곳 소개글과 108곳 지점별 오디오 가이드를 6개 국어(한·영·서·프·포·이)로 직접 집필, 성지 DB 안에서만 답하는 AI 가이드.',
    '4) 다녀온 성지에 한 줄과 사진을 남기는 「순례 기록」을 서비스 중심(하단 탭 가운데)에 둔다 — 기록이 쌓이면 개인별 다음 성지 제안과 지역 분산 측정의 근거가 된다.',
...
HASHTAGS = [
    ('#오늘의성지일정', '마음 상태로 고르는 하루 일정 (오버투어리즘 대응의 중심)',
     ['질문 3개(마음 · 출발지 · 시간)에 답하면 반경 안 후보 성지 3곳을 보여 주고, 하나를 고르면 오전 성지 → 점심 → 오후 관광지의 하루 일정을 짠다.',
      ...
      '시간 답(반나절·하루·1박2일)이 출발지 반경(30km·80km·전국)이 되어 이동이 체류보다 길어지지 않게 한다. 성지 상세에서도 같은 집중률로 「주변 밀집도 상·중·하」를 보여 준다.']),
    ('#오디오가이드', '성지 이야기 · 지점별 오디오 가이드 (6개 국어)',
     [f'성지 {SITES}곳마다 3문단 소개글(순교·신앙 역사 / 위치·지리 / 건축)을 6개 국어로 직접 집필해 DB 에 두었다(2026-09-20 실측 206곳 × 6개 국어 완비).',
      '현장 지점 원고(여는 말 → 지점 3~6곳 → 맺음말)는 108곳 × 6개 국어. 기기 TTS 로 읽어 주며 속도 조절·챕터 이동이 되고, 원고가 없는 성지는 소개·역사 낭독으로 대체한다.',
      '모든 원고에 출처를 붙이고, 관광공사 두루누비 걷기길·주변 관광지(둘러볼 곳)와 나란히 보여 준다. 상단 지구본 버튼으로 언어를 바꾸면 화면·이야기·가이드가 함께 바뀐다.']),
    ...
    ('#순례기록', '다녀온 성지에 한 줄과 사진 — 함께 만드는 순례 안내',
     ['성지 상세의 「순례 기록 남기기」로 방문일·한 줄 메모·사진(성지당 1개)을 남긴다. 다른 순례자에게 익명으로 공개되어 「순례 기록 (N)」으로 쌓이고, 신고로 걸러진다.',
      '하단 탭 가운데 「기록」에서 내 기록과 즐겨찾는 성지를 모아 보고 수정·삭제한다. 순례 코스 상세는 기록을 기준으로 「n화까지 기록」 진행을 보여 준다.',
      '기록은 우리 데이터라 분석할 수 있다 — 기록의 성지 → 지역으로 비수도권 방문 비율을 잴 수 있고, 다음 단계로 기록 저장 때 「한적했어요/보통/붐볐어요」를 받아 집중률 예측을 보정한다(발전계획).']),
]
sh4 = table_of(S[3])
t = sh4.table
...
FLOWS = {
    '#오늘의성지일정': [
        ...
        ('compass-plan-390.png', '오전 성지 → 점심 → 오후 관광지. 관광공사 데이터를 지금 불러온 일정'),
    ],
    '#오디오가이드': [
    ...
    '#순례가이드미카엘': [
        ...
        ('menu-390.png', '더보기 — 언어 6개·글자 크기·홈 화면 추가·계정 설정'),
    ],
    '#순례기록': [
        ('detail-record-390.png', '성지 상세 「순례 기록」 — 다녀온 사람들의 한 줄과 사진'),
        ('records-390.png', '하단 탭 가운데 「기록」 — 로그인하면 내 기록·즐겨찾기'),
...
imgs = ['home-390.png', 'compass-plan-390.png', 'detail-top-390.png', 'detail-docent-390.png', 'michael-390.png', 'records-390.png']
cw = w // len(imgs)
...
OTHERS = [
    (f'자체 구축 성지 DB (holy_sites · holy_site_translations · docent_scripts, {SITES}곳)',
     f'교구 홈페이지·굿뉴스 성지 안내·현장 취재로 직접 수집한 성지 {SITES}곳의 주소·연락처·미사 시간·역사와, 성지마다 6개 국어 소개글({SITES}곳 × 6) + 지점별 오디오 가이드 원고(108곳 × 6). 대표 사진은 직접 촬영·교구 홍보국 제공분(출처·라이선스 표기). Supabase(PostgreSQL) 저장.'),
    ...
    ('순례 기록 (pilgrimage_stamps · 이용자 생성)',
     '이용자가 남긴 방문일·한 줄·사진. 우리 데이터라 분석 가능 — 비수도권 방문 비율 측정, 다음 성지 제안, 집중률 예측 보정(발전계획)의 재료.'),
...
set_text(t.cell(0, 1), [
    '• 「대체 관광지 추천」이 아니라 「오늘의 하루 일정」 — 집중률 예측·주변 관광지를 그날 실시간으로 조합해 붐빔을 피하면서도 그 지역에 머물게 한다. 성지 68%가 비수도권이라 분산이 구조적으로 일어난다.',
    f'• 콘텐츠를 직접 만들었다 — 성지 {SITES}곳 소개글과 108곳 오디오 가이드를 6개 국어로, 모든 원고에 출처 표기. 기존 관광 앱이 다루지 않는 종교 유산 층위.',
    '• AI 가 지어내지 않는다 — 미카엘은 성지 DB 안에서만 답하고 참고한 성지를 밝힌다. 종교·역사 정보의 신뢰가 서비스의 생명이라는 판단.',
    '• 순례 기록이 서비스의 중심 — 다녀온 사람의 한 줄과 사진이 다음 순례자의 안내가 되고, 쌓일수록 분산 효과를 잴 수 있다.',
    '• 시니어·외국인 접근성 — 큰 글자 모드, 6개 국어 UI·이야기·가이드, 기기 TTS, 홈 화면 설치(PWA), 카카오·네이버·구글 간편 로그인, 접근성 감사(WCAG 2.1 AA) 반영.',
    '• 관광공사 데이터 원칙 준수 — TourAPI 응답을 DB·캐시에 저장하지 않고 매 요청 실시간 호출(ADR 0002 로 문서화, 코드로 강제).',
], 10)
set_text(t.cell(1, 1), [
    '• 시간 분산: 집중률 예측 30일치를 「이번 달 조용한 날」 달력으로 — 이미 받는 데이터를 하루에서 한 달로. 전례력(순교자 성월·축일 미사)과 함께 표시.',
    '• 실측 보정: 순례 기록 저장 때 「한적했어요/보통/붐볐어요」 한 칸을 받아 예측치를 보정 — 쓸수록 정확해지는 구조. 기록 기반 「다음 성지 제안」(수도권 기록이 많으면 지방 성지).',
    '• 공간 분산 측정: 기록의 성지 → 지역으로 비수도권 방문 비율 대시보드. 교구별 완주 배지(성지가 적은 교구부터).',
    '• 지방 성지 1박 2일 묶음(2일차 일정) + 대중교통·순례버스 접근 정보. 오디오 가이드 나머지 98곳 집필, 대표 사진 전 성지 확보(현재 102곳).',
    '• 2027 서울 WYD 대비 교구·본당 미사 시간 연동(CBCK 주소록), 시·도 랜딩 페이지로 지자체·교구 협업, Android/iOS 스토어 출시(Capacitor).',
], 10)
