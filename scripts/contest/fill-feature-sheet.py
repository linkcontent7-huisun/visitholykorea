...
set_text(t.cell(0, 1), [
    ...
    '**2) 공간 분산(어디로)** — 관광공사 「관광지 집중률 예측」을 매 요청 실시간으로 받아 성지마다 **「주변 밀집도 상·중·하」**(숫자 대신 세 단계, 50대 이상도 한눈에). 「오늘의 성지 일정」은 마음·출발지·시간(질문 3개)에 맞는 후보 중 **밀집도 낮은 성지를 앞세우고**, 오후가 붐빌 예정이면 같은 지역의 다른 관광지를 제안.',
    '**3) 체류(얼마나 오래)** — 고른 성지를 축으로 **오전 성지 → 점심(주변 식당) → 오후(주변 관광지·걷기길)** 하루 일정을 TourAPI 실시간 조회로(응답 저장 안 함). 순례 코스 11개로 1곳 방문을 여러 곳·여러 날로.',
    f'**4) 현장 콘텐츠** — 성지 {SITES}곳 소개글과 108곳 지점별 오디오 가이드를 **6개 국어로 직접 집필**, 성지 DB 안에서만 답하는 **AI 가이드** — 지방의 덜 알려진 성지도 "갈 만한 곳"으로.',
    '**5) 시간 분산(언제)과 측정** — 집중률 예측 30일치를 **「한적한 날」** 카드로 펼쳐 "주말 말고 이 날에". **「순례 기록」**을 서비스 중심에 두고 저장 때 **「그날 붐볐나요?」**를 받아 예측을 실측으로 보정, 기록의 성지 → 지역으로 **비수도권 방문 비율을 측정**.',
], 13)
set_text(t.cell(1, 1), [
...
HASHTAGS = [
    ('#오늘의성지일정', '마음 상태로 고르는 하루 일정 — 오버투어리즘 대응의 중심',
     ['• **질문 3개**(마음 · 출발지 · 시간) → 반경 안 **후보 성지 3곳** → 하나를 고르면 **오전 성지 → 점심 → 오후 관광지** 하루 일정',
      '• TourAPI **관광지 집중률 예측**으로 「인근이 조용해요」 후보 표시와 「인근 지역이 조용해요/보통이에요/붐벼요」, locationBasedList2 로 주변 식당·관광지 **실시간** 조회. 오후가 붐빌 예정이면 머무르기 또는 **대안 관광지 제안**',
...
APIS = [
    ...
    ('국문 관광정보 서비스 (KorService2)',
     'locationBasedList2 로 성지 주변 **관광지·식당**(하루 일정 점심·오후, 성지 상세 「둘러볼 곳」), searchKeyword2·areaBasedList2 로 검색, ldongCode2 로 시·군·구 코드. **매 화면 실시간 호출**, DB·서비스워커에 저장하지 않음.'),
    ('영문·서어·불어 관광정보 서비스 (EngService2 · SpnService2 · FreService2)',
     '앱 언어가 영어·스페인어·프랑스어면 주변 관광지를 **그 언어 서비스**로 조회(포르투갈어·이탈리아어는 영어로). 결과가 비면 국문으로 폴백 — **외국인이 읽을 수 있는 주변 정보**.'),
    ('지역 허브 관광지 · 연관 관광지 (LocgoHubTarService1 · TarRlteTarService1)',
     '하루 일정의 오후가 붐빌 예정일 때 같은 시·군·구의 **대안 관광지**를 고른다 — 붐비는 곳 대신 근처의 다른 곳으로 **공간 분산**.'),
...
OTHERS = [
    (f'자체 구축 성지 DB (holy_sites · holy_site_translations · docent_scripts, {SITES}곳)',
     f'각 교구 홈페이지·굿뉴스 성지 안내·가톨릭 언론의 공개 자료와 현장에서 수집한 주보·안내문을 대조해 **직접 쓴** 성지 {SITES}곳의 주소·연락처·미사 시간·역사. 성지마다 **6개 국어 소개글({SITES}×6)**과 **지점별 오디오 가이드(108×6)**. 대전교구 성지는 교구 홍보국 제공 자료 참고. 성지 이야기에 **출처 링크** 표시. 대표 사진은 직접 촬영·교구 제공분(출처·라이선스 표기). Supabase(PostgreSQL).'),
...
set_text(t.cell(0, 1), [
    '• **웹 구현** — 반응형(휴대폰 하단 탭·PC 상단 메뉴를 **한 코드**로, 1024px 자동 전환) · PWA 홈 화면 설치 · **6개 국어** 화면·이야기·가이드 · 큰 글자 모드 · 접근성(WCAG 2.1 AA) 감사 반영 · 카카오·네이버·구글 로그인',
    ...
    '• **함께 만드는 순례 기록** — 다녀온 사람의 한 줄과 사진이 다음 순례자의 안내가 되고, 「그날 붐볐나요?」가 쌓일수록 **예측이 실측으로 보정**된다',
