...
rows = [
    ...
    ([f'유명 성지 열 곳·주말·수도권에 몰리는 순례를 **{SITES}곳(68% 비수도권)·30일 예측·지방 체류**로 펼치는 서비스.',
      f'성지 **{SITES}곳**을 "오늘 마음·출발지·시간"에 맞춰 골라 주고, TourAPI 데이터(**매일 갱신되는 집중률 예측**·주변 관광지·걷기길)를 그때그때 호출해 **하루 일정**을 짜며,',
      '현장에서는 **6개 국어 성지 이야기와 오디오 가이드**로 안내하고, 다녀온 **순례 기록**을 함께 남긴다'], 14),
    ...
    ([f'• 성지 {SITES}곳 중 **140곳(68%)이 비수도권**(2026-09-21 DB 실측) — 목적지 목록 자체가 분산을 돕는다',
      ...
      '• 관광공사 **집중률 30일 예측을 그때그때 호출해** 붐비는 날·곳을 피해 **같은 지역의 조용한 성지**와 볼거리를 제안',
      f'• 유명 성지 열 곳에 몰리던 순례를 **공간({SITES}곳)·시간(30일 예측)·체류(주변 식당·걷기길)**로 펼친다'], 13),
]
...
set_text(t.cell(0, 1), [
    ...
    '**2) 공간 분산(어디로)** — 관광공사 「관광지 집중률 예측」(매일 갱신)을 호출 때마다 받아(저장 안 함) 성지마다 **「주변 밀집도 상·중·하」**(숫자 대신 세 단계, 50대 이상도 한눈에). 「오늘의 성지 일정」은 마음·출발지·시간(질문 3개)에 맞는 가까운 후보 3곳에 **「인근이 조용해요」 표시로 밀집도 낮은 성지를 고르게** 하고, 오후 관광지의 당일 집중률 예측이 붐빔 등급(70 이상)이면 같은 지역의 다른 관광지를 제안.',
    '**3) 체류(얼마나 오래)** — 고른 성지를 축으로 **오전 성지 → 점심(주변 식당) → 오후(주변 관광지·걷기길)** 하루 일정을 TourAPI 를 그때그때 조회해(응답 저장 안 함). 순례 코스 11개(6개 국어)로 1곳 방문을 여러 곳·여러 날로.',
    ...
    '**공고 예시 기능 대응** — 조건 설정(출발지·시간·마음 3문항) → 대안 장소(후보 3곳 「인근이 조용해요」 + 30일 「한적한 날」) → 코스 추천(오전 성지 → 점심 → 오후, 순례 코스 11개)',
], 13)
set_text(t.cell(1, 1), [
...
HASHTAGS = [
    ('#오늘의성지일정', '마음 상태로 고르는 하루 일정 — 오버투어리즘 대응의 중심',
     ['• **질문 3개**(마음 · 출발지 · 시간) → 반경 안 **후보 성지 3곳** → 하나를 고르면 **오전 성지 → 점심 → 오후 관광지** 하루 일정',
      '• TourAPI **관광지 집중률 예측**으로 「인근이 조용해요」 후보 표시와 「인근 지역이 조용해요/보통이에요/붐벼요」, locationBasedList2 로 주변 식당·관광지를 그때그때 조회. 오후 관광지의 당일 집중률 예측이 붐빔 등급(70 이상)이면 성지에 더 머무르기 또는 **같은 반경의 다른 관광지로 바꾸기** 제안',
      '• 시간 답(반나절·하루·1박2일) = 출발지 반경 **20·60·180km** — 이동이 체류보다 길어지지 않게. 성지 상세의 **「주변 밀집도 상·중·하」**와 30일 예측 중 조용한 날짜를 고른 **「한적한 날」** 카드 — 붐비는 날을 피하는 시간 분산']),
    ('#오디오가이드', '지방의 덜 알려진 성지를 갈 만한 곳으로 — 206곳 이야기·108곳 가이드 6개 국어',
     [f'• 성지 **{SITES}곳** 소개글(위치·역사·건축·주변 이야기 3문단)을 **6개 국어**(한·영·서·프·포·이)로 직접 집필 — 2026-09-21 실측 {SITES} × 6 완비',
      ...
      '• 성지 이야기에 **참고한 출처 링크** 표시(교구 공식 자료·굿뉴스 성지 안내). 두루누비 걷기길·둘러볼 곳(관광공사, 그때그때 조회)과 나란히. 지구본 버튼 하나로 **화면·이야기·가이드가 함께 언어 전환**']),
    ('#순례가이드미카엘', '정보가 적은 지방 성지의 주소·미사·전화를 DB 근거로 답하는 AI',
    ...
    ('#순례기록', '분산을 측정하는 기록 — 「그날 붐볐나요?」와 비수도권 방문 비율',
...
FLOWS = {
    ...
    '#오디오가이드': [
        ...
        ('detail-nearby-390.png', '「둘러볼 곳」 — 관광공사 식당·볼거리를 그때그때 조회해 체류 연장'),
    ],
    '#순례가이드미카엘': [
...
APIS = [
    ...
    ('국문 관광정보 서비스 (KorService2)',
     'locationBasedList2 로 성지 주변 **관광지·식당**을 받아 성지 상세 「둘러볼 곳」(걸어갈 1km, 비면 3km)과 하루 일정의 점심·오후(반경 5km, 붐빌 예정이면 같은 풀의 다음 후보로 대안)를 채우고, ldongCode2 로 집중률 조회에 필요한 시·군·구 코드를 받는다. **실시간 호출**(응답은 화면 메모리에서만 잠시 유지), DB·서비스워커에 저장하지 않음.'),
...
set_text(t.cell(0, 1), [
    f'• **「대체 관광지 추천」이 아니라 「오늘의 하루 일정」 + 「한적한 날」** — 집중률 예측(일 단위, 30일치)을 조합해 **붐비는 곳과 붐비는 날을 모두 피하면서 그 지역에 머물게** 한다. 성지 **68%(140/{SITES})가 비수도권**이라 분산이 구조적으로 일어난다',
