...
set_text(t.cell(0, 1), [
    ...
    '5) 측정과 다음 단계(언제) — 「순례 기록」(한 줄·사진)을 서비스 중심에 두어 기록의 성지 → 지역으로 비수도권 방문 비율을 잴 수 있는 장치를 갖췄다. 다음 단계는 이미 받고 있는 집중률 30일치를 「이번 달 조용한 날」 달력으로 펼치는 시간 분산과, 기록 저장 때 「한적했어요/보통/붐볐어요」를 받아 예측을 보정하는 것이다.',
], 13)
set_text(t.cell(1, 1), [
...
HASHTAGS = [
    ('#오늘의성지일정', '마음 상태로 고르는 하루 일정 (오버투어리즘 대응의 중심)',
     ['질문 3개(마음 · 출발지 · 시간)에 답하면 반경 안 후보 성지 3곳을 보여 주고, 하나를 고르면 오전 성지 → 점심 → 오후 관광지의 하루 일정을 짠다.',
      ...
      '시간 답(반나절·하루·1박2일)이 출발지 반경(20km·60km·180km)이 되어 이동이 체류보다 길어지지 않게 한다. 성지 상세에서도 같은 집중률로 「주변 밀집도 상·중·하」를 보여 준다.']),
    ...
    ('#순례기록', '함께 만드는 순례 기록 — 내 한 줄이 다음 순례자의 안내가 된다',
     ['성지 상세의 「순례 기록 남기기」로 방문일·한 줄 메모·사진(최대 3장)을 남긴다. 기록은 그 성지 상세 페이지 아래에 다른 순례자의 후기로 쌓여 「순례 기록 (N)」이 된다 — 운영자가 쓰는 안내가 아니라 모두가 함께 만들어 가는 순례 안내. 익명 공개, 신고로 걸러진다.',
      ...
      '기록은 우리 데이터로 남는다 — 기록의 성지 → 지역으로 비수도권 방문 비율을 재고, 기록 저장 때 「한적했어요/보통/붐볐어요」를 받아 관광공사 집중률 예측을 실측으로 보정하는 데 쓴다(발전계획).']),
]
sh4 = table_of(S[3])
t = sh4.table
...
set_text(t.cell(0, 1), [
    ...
    f'• 「대체 관광지 추천」이 아니라 「오늘의 하루 일정」 — 집중률 예측·주변 관광지를 그날 실시간으로 조합해 붐빔을 피하면서도 그 지역에 머물게 한다. 성지 68%(140/{SITES})가 비수도권이라 분산이 구조적으로 일어난다.',
...
set_text(t.cell(1, 1), [
    '• 시간 분산 — 이미 받고 있는 집중률 예측 30일치를 「이번 달 조용한 날」 달력으로 펼친다("주말 말고 화요일에"). 전례력(순교자 성월·성지 축일 미사)과 함께 표시.',
    '• 실측 보정 — 순례 기록 저장 때 「한적했어요/보통/붐볐어요」 한 칸을 받아 관광공사 예측치를 실데이터로 보정, 쓸수록 정확해지는 구조. 같은 성지 재방문 기록 허용.',
