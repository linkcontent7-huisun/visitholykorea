...
rows = [
    ...
    [f'전국 천주교 성지 {SITES}곳을 "오늘 마음"에 맞춰 골라 주고, 한국관광공사 TourAPI 실시간 데이터(집중률·축제·주변 관광지·걷기길)로 '
     '하루 일정을 짜 주며, 현장에서는 6개 국어 오디오 가이드로 안내하는 순례 여행 서비스'],
...
set_text(t.cell(0, 1), [
    ...
    '4) 현장 체류의 질을 높이는 콘텐츠 — 성지마다 직접 쓴 소개글과 지점별 오디오 가이드(6개 국어), 성지 DB 안에서만 답하는 AI 가이드.',
    '5) 박해사·인물을 축으로 성지를 잇는 순례 코스로 1곳 방문을 여러 곳·여러 날로 늘린다.',
], 13)
set_text(t.cell(1, 1), [
    '#오늘의성지일정  #오디오가이드  #순례가이드미카엘  #순례코스',
], 16, bold_first=True)
...
HASHTAGS = [
    ...
    ('#순례코스', '박해사·인물 축 순례 코스',
     ['「내포, 신앙의 못자리 길」 「내포 도보 순례길 ① 솔뫼에서 해미까지(40km)」처럼 사건·인물로 성지 4~6곳을 잇는 코스를 제공한다.',
      ...
      '코스 상세에서 각 성지 상세(성지 이야기·오디오 가이드·둘러볼 곳)로 바로 이어진다.']),
]
sh4 = table_of(S[3])
t = sh4.table
...
APIS = [
    ('국문 관광정보 서비스 (KorService2)',
     'locationBasedList2 로 성지 주변 관광지·식당(하루 일정 점심·오후, 성지 상세 「둘러볼 곳」), searchFestival2 로 오늘 진행 중인 전국 축제(인근 혼잡도 산식의 30%), '
...
OTHERS = [
    (f'자체 구축 성지 DB (holy_sites · docent_scripts, {SITES}곳)',
     f'교구 홈페이지·굿뉴스 성지 안내·현장 취재로 직접 수집한 성지 {SITES}곳의 주소·연락처·미사 시간·역사와, 성지마다 6개 국어 소개글 + 지점별 오디오 가이드 원고. 대표 사진은 직접 촬영·교구 홍보국 제공분(출처·라이선스 표기). Supabase(PostgreSQL) 저장.'),
...
set_text(t.cell(0, 1), [
    ...
    f'• 콘텐츠를 직접 만들었다 — 성지 {SITES}곳 소개글 6개 국어, 지점별 오디오 가이드 원고, 출처 URL 을 모든 원고에 표기. 기존 관광 앱이 다루지 않는 종교 유산 층위.',
