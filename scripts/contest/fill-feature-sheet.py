...
APIS = [
    ...
    ('국문 관광정보 서비스 (KorService2)',
     'locationBasedList2 로 성지 반경 5km 의 **관광지·식당**을 받아 성지 상세 「둘러볼 곳」과 하루 일정의 점심·오후(붐빌 예정이면 같은 풀의 다음 후보로 대안)를 채우고, ldongCode2 로 집중률 조회에 필요한 시·군·구 코드를 받는다. **매 화면 실시간 호출**, DB·서비스워커에 저장하지 않음.'),
...
set_text(t.cell(0, 1), [
    '• **웹 구현** — 반응형(휴대폰 하단 탭·PC 상단 메뉴를 **한 코드**로, 1024px 자동 전환) · PWA 홈 화면 설치 · **6개 국어** 화면·이야기·가이드 · 큰 글자 모드 · 접근성 정적 검토(WCAG 2.1 기준) 상위 항목 반영 · 카카오·네이버·구글 로그인',
