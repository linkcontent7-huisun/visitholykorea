...
interface Row {
  site_id: string; language: string; kind: 'intro' | 'point'; seq: number;
  title: string | null; body: string; look_for: string | null; sources: unknown; status: string; written_by: string | null;
}
...
if (existsSync(introDir)) {
  for (const f of readdirSync(introDir)) {
    ...
    else if (existsSync(join(full, '.'))) for (const g of readdirSync(full)) if (g.endsWith('.md')) introFiles.push(join(full, g)); // 언어별 하위 폴더(en/ …)
  }
...
for (const f of readdirSync(DIR).filter((n) => n.endsWith('.json') && !n.startsWith('_') && n.includes(filter))) {
  ...
  for (const [lang, sfx] of langs) {
    ...
    rows.push({ site_id: d.siteId, language: lang, kind: 'point', seq: 0, title: lang === 'ko' ? '여는 말' : 'Welcome', body: intro, look_for: null, sources: src, status: d.status === 'verified' ? 'verified' : 'draft', written_by: d.surveyedBy ?? null });
    for (const p of pts) rows.push({ site_id: d.siteId, language: lang, kind: 'point', seq: p.seq, title: p[`title${sfx}`] ?? p.title, body: p[`narration${sfx}`], look_for: p[`lookFor${sfx}`] ?? null,
      sources: p.sourceNote ? [{ label: p.sourceNote }] : src, status: d.status === 'verified' ? 'verified' : 'draft', written_by: d.surveyedBy ?? null });
    if (outro) rows.push({ site_id: d.siteId, language: lang, kind: 'point', seq: 99, title: lang === 'ko' ? '맺음말' : 'Farewell', body: outro, look_for: null, sources: src, status: d.status === 'verified' ? 'verified' : 'draft', written_by: d.surveyedBy ?? null });
  }
  console.log(`지점    ${f}  ${(d.points ?? []).length}지점`);
}

if (rows.length === 0) { console.log('올릴 원고가 없다'); process.exit(0); }
...
for (const r of rows) {
  await client.query(
    ...
    [r.site_id, r.language, r.kind, r.seq, r.title, r.body, r.look_for, JSON.stringify(r.sources), r.status, r.written_by],
  );
  n += 1;
}
const c = await client.query(`select language, kind, count(*) n, count(distinct site_id) sites from public.docent_scripts group by 1,2 order by 1,2`);
await client.end();
console.log(`\n${n}행 올림. 표 전체:`);
