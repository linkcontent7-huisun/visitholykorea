...
async function main() {
  ...
  try {
    for (const row of rows()) {
      ...
      if (existing) {
        ...
        const updates: Record<string, unknown> = {
          cbck_code: row.cbck_code,
          cbck_synced_at: new Date().toISOString(),
        };
        for (const key of columns.slice(1))
          if (!filled(existing[key]) && filled(row[key])) updates[key] = row[key];
        ...
        if (!DRY_RUN) {
          ...
          await db.query(
            `update catholic_directory set ${keys.map((key, i) => `${key} = $${i + 1}`).join(', ')} where id = $${keys.length + 1}`,
        ...
        if (!DRY_RUN) {
          ...
          await db.query(
            `insert into catholic_directory (${keys.join(', ')}) values (${keys.map((_, i) => `$${i + 1}`).join(', ')})`,
      ...
      if (row.kind === '성지사적지') {
        const sites = await db.query<{
          ...
          `select id, phone, homepage_url, fax from holy_sites where name_compact = replace($1, ' ', '') or name_compact like '%' || replace($1, ' ', '') || '%' or replace($1, ' ', '') like '%' || name_compact || '%'`,
          [row.name],
        );
        for (const site of sites.rows) {
          const changes: Record<string, unknown> = {};
          if (!filled(site.phone) && filled(row.phone)) changes.phone = row.phone;
          if (!filled(site.homepage_url) && filled(row.homepage))
            changes.homepage_url = row.homepage;
          ...
          if (Object.keys(changes).length) {
            ...
            if (!DRY_RUN) {
              ...
              await db.query(
                `update holy_sites set ${keys.map((key, i) => `${key} = $${i + 1}`).join(', ')} where id = $${keys.length + 1}`,
                [...keys.map((key) => changes[key]), site.id],
              );
              await db.query(
                `insert into site_sources (site_id, kind, title, url, note) values ($1, 'web', '한국 천주교 주소록 (directory.cbck.or.kr)', $2, '전화·홈페이지·팩스') on conflict (site_id, kind, title) do nothing`,
                [site.id, row._detail_url ?? null],
              );
            }
          }
        }
    ...
    console.log(
      `${DRY_RUN ? '예정' : '완료'}: 기존 갱신 ${updated}, 기존 매칭 ${matched}, 신규 ${inserted}, holy_sites 보강 ${holyPlanned}`,
    );
  } finally {
    await db.end();
  }
