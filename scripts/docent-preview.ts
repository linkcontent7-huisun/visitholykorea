...
const intro = await c.query(`select h.name, h.diocese, h.image_url, s.body, s.status, s.sources, s.written_by, s.updated_at from docent_scripts s join holy_sites h on h.id=s.site_id where s.kind='intro' and s.language='ko' order by s.updated_at desc, h.name`);
const pts = await c.query(`select h.name, s.seq, s.title, s.body, s.look_for from docent_scripts s join holy_sites h on h.id=s.site_id where s.kind='point' and s.language='ko' order by h.name, s.seq`);
await c.end();
const esc = (t: string) => t.replace(/&/g,'&amp;').replace(/</g,'&lt;');
const byName: Record<string, typeof pts.rows> = {};
for (const p of pts.rows) (byName[p.name] ??= []).push(p);
const img = (u: string|null) => !u ? '' : u.startsWith('http') ? u : 'http://127.0.0.1:5180/public' + u;
...
for (const r of intro.rows) {
  ...
  if (p) { html += `<div class="pts"><div class="lbl">지금 이야기 — 지점 ${p.filter(x=>x.seq>0&&x.seq<99).length}개</div>`; for (const x of p) html += `<div class="p"><b>${x.seq===0?'여는 말':x.seq===99?'맺음말':x.seq+'. '+esc(x.title??'')}</b>${esc(x.body)}${x.look_for?`<i>눈여겨볼 것: ${esc(x.look_for)}</i>`:''}</div>`; html += `</div>`; }
  html += `</section>`;
}
html += `<a class="top" href="#top">맨 위 ↑</a></div></body></html>`;
mkdirSync('screenshots', { recursive: true }); writeFileSync('screenshots/docent-preview.html', html);
