...
function load(dir: string) {
  if (!existsSync(dir))
    return [] as { name: string; head: string; paras: string[]; body: string }[];
  return readdirSync(dir)
    .filter((f) => f.endsWith('.md') && f.includes(only))
    .map((f) => {
...
function check(files: ReturnType<typeof load>, lang: Lang) {
  ...
  const template = new Set(
    [...sentenceOwners.entries()].filter(([, o]) => o.size >= 3).map(([k]) => k),
  );
  const problems: string[] = [];
  const endsOk = lang === 'ko' ? /(니다|요)\."$/ : /[.!?]"$/; // 한국어는 존댓말로 끝나야 한다 — "~자리다." 같은 반말 금지
  const hasFact =
    lang === 'ko'
      ? /\d{3,4}|신부|성인|주교|성당|양식|벽돌|고딕|로마네스크|순교|성지|공소|교우촌|박해|주교좌|수도원|기념/
      : lang === 'en'
        ? /\d{3,4}|Father|Bishop|Saint|St\.|martyr|Gothic|Romanesque|brick|church|cathedral|shrine|hill|village|gate|parish|persecution|mission/i
        : LATIN_FACT;
  for (const f of files) {
    const bad: string[] = [];
    ...
    f.paras.forEach((p, i) => {
      if (!endsOk.test(p)) bad.push(`${i + 1}문단 잘림`);
    });
    f.paras.forEach((p, i) => {
      if (!hasFact.test(p)) bad.push(`${i + 1}문단 고유 사실 없음`);
    });
    ...
    for (const sent of f.body.split(/(?<=[.。!?"])\s+/)) {
      const k = sent.replace(/^"|"$/g, '').replace(/\s+/g, ' ').trim();
      if (k.length >= 15) seen.set(k, (seen.get(k) ?? 0) + 1);
    }
    const rep = [...seen.entries()].filter(([, c]) => c >= 2);
    if (rep.length) bad.push(`파일 안 반복 ${rep.length}문장`);
    if (
      lang === 'ko' &&
      /[가-힣](다|자)\.\s/.test(f.body.replace(/(니다|습니다|입니다)\.\s/g, ' '))
    )
      bad.push('반말 문장');
...
if (ko.template.size) {
  console.log('틀 문장:');
  [...ko.template].slice(0, 5).forEach((t) => console.log(`  - ${t.slice(0, 60)}`));
}
const NAMES: Record<Exclude<Lang, 'ko'>, string> = {
  en: '영어',
  es: '스페인어',
  it: '이탈리아어',
  fr: '프랑스어',
