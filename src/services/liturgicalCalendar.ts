...
const SEASON_STYLE: Record<LiturgicalSeasonKey, { emoji: string; colorClass: LiturgicalEvent['colorClass'] }> = {
  대림: { emoji: '🕯️', colorClass: { bg: 'bg-violet-500', ring: 'ring-violet-300', text: 'text-violet-600' } },
  성탄: { emoji: '⭐', colorClass: { bg: 'bg-amber-400', ring: 'ring-amber-200', text: 'text-amber-600' } },
  사순: { emoji: '✝️', colorClass: { bg: 'bg-purple-700', ring: 'ring-purple-300', text: 'text-purple-700' } },
  부활: { emoji: '🐣', colorClass: { bg: 'bg-yellow-400', ring: 'ring-yellow-200', text: 'text-yellow-600' } },
  연중: { emoji: '🌿', colorClass: { bg: 'bg-emerald-600', ring: 'ring-emerald-200', text: 'text-emerald-600' } },
};

const SPECIAL_MONTH_STYLE: Record<Exclude<SpecialMonthKey, null>, { emoji: string; colorClass: LiturgicalEvent['colorClass'] }> = {
  성모성월: { emoji: '🌹', colorClass: { bg: 'bg-sky-500', ring: 'ring-sky-200', text: 'text-sky-600' } },
  성심성월: { emoji: '❤️', colorClass: { bg: 'bg-red-500', ring: 'ring-red-200', text: 'text-red-600' } },
  순교자성월: { emoji: '🩸', colorClass: { bg: 'bg-rose-700', ring: 'ring-rose-300', text: 'text-rose-700' } },
  위령성월: { emoji: '🕊️', colorClass: { bg: 'bg-slate-600', ring: 'ring-slate-300', text: 'text-slate-600' } },
};
...
export function getLiturgicalEvent(date: Date = new Date()): LiturgicalEvent {
  ...
  if (specialMonth) {
    ...
    return { season, specialMonth, label: specialMonth, emoji: style.emoji, colorClass: style.colorClass };
  }

  const style = SEASON_STYLE[season];
  return { season, specialMonth: null, label: `${season} 시기`, emoji: style.emoji, colorClass: style.colorClass };
}
