...
function buildCertificateCanvas(options: CertificateOptions): HTMLCanvasElement {
  ...
  // 발급일
  const dateStr = options.issuedAt.toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' });
...
export function downloadCertificatePDF(options: CertificateOptions) {
