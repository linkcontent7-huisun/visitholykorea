 ...
 *   2. 저장된 선택이 노출 목록 밖이면 영어 (언어를 잠시 내렸을 때를 위한 안전장치)
 *   3. 처음 온 사람은 브라우저 언어가 노출 목록에 있으면 그 언어, 없으면 영어
 *      (2026-09-20 6개 국어를 켜면서 — 스페인어 브라우저는 스페인어로 시작한다)
...
export function resolveInitialLanguage(
  ...
  return ENABLED_LANGUAGES.find((lang) => first.startsWith(lang)) ?? 'en';
}
