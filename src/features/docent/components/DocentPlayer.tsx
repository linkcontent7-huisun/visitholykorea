    ...
    <div className="mb-6 overflow-hidden rounded-[28px] border border-app-border bg-app-bg">
      <div className="flex items-center gap-3 border-b border-app-border p-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-[0.6875rem] font-medium text-app-text-muted">
            {t('docentHint')}
          </p>
          {/* 문헌으로만 쓴 원고임을 정직하게 표시한다 — 더미 금지 원칙.
              좁은 화면에서 제목을 밀어내지 않도록 배지는 제목 아래 줄에 둔다 */}
          {isDraft && (
            <span className="mt-1 inline-block rounded-full bg-amber-100 px-2.5 py-0.5 text-[0.625rem] font-extrabold text-amber-700">
              {t('docentDraft')}
            </span>
          )}
        </div>
        ...
        {isSupported && (
          <button
            onClick={cycleRate}
            className="flex shrink-0 items-center gap-1 rounded-full border border-app-border bg-white px-2.5 py-1 text-[0.625rem] font-extrabold text-app-text-muted"
      ...
      {!isSupported && (
        <div className="border-b border-app-border bg-sky-50 px-5 py-3">
          <p className="text-[0.6875rem] font-medium leading-relaxed text-sky-800">
            {t('docentUnsupported')}
          </p>
        </div>
      )}
      ...
      {headphoneGate && (
        <div className="border-b border-app-border bg-violet-50 px-5 py-4">
          <p className="flex items-start gap-2 text-[0.75rem] font-medium leading-relaxed text-violet-900">
          ...
          <>
            <button
              ...
              {isAndroid() && (
                <p className="mt-2 text-[0.625rem] font-medium leading-relaxed text-violet-700/70">
      ...
      {isSupported && isVoiceMissing && (
        <div className="border-b border-app-border bg-amber-50 px-5 py-3">
          <p className="text-[0.6875rem] font-medium leading-relaxed text-amber-800">
      ...
      {hasError && !isVoiceMissing && (
        <div className="border-b border-app-border bg-amber-50 px-5 py-3">
          <p className="text-[0.6875rem] font-medium leading-relaxed text-amber-800">{t('docentError')}</p>
        </div>
      )}

      {/* 지금 읽는 챕터의 전문 — 소리를 켤 수 없는 곳, 잘 들리지 않는 이들을 위해 */}
      <div className="border-b border-app-border bg-white/60 px-5 py-4">
        <p className="text-[0.8125rem] font-medium leading-relaxed text-app-text">
          {current.narration}
        </p>
      </div>
        ...
        {chapters.map((chapter, i) => {
          ...
          return (
            <li key={chapter.id} className="border-b border-app-border/60 last:border-b-0">
              ...
              >
                <span
                  className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[0.6875rem] font-extrabold ${
                    isCurrent
                      ? 'bg-brand-violet text-white'
                ...
                <span className="min-w-0 flex-1">
                  ...
                  {chapter.location && (
                    <span className="mt-1 flex items-start gap-1 text-[0.6875rem] font-medium leading-relaxed text-app-text-muted">
                      <MapPin size={12} className="mt-0.5 shrink-0" />
                      {t('docentLocation')}: {chapter.location}
                    </span>
                  )}
                  {chapter.lookFor && (
                    <span className="mt-0.5 block text-[0.6875rem] font-medium leading-relaxed text-app-text-muted">
